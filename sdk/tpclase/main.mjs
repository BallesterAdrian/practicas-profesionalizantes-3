       ...
       let body = '';
        ...
        request.on('end', async () => 
        {
            try 
            {
                ...
                const output = login(username, password); //El resultado es nulo o un objeto de sesión
                ...
                if (output != null)
                    {
                        ...
                        response.end(`
                            <h1>Sesion iniciada correctamente</h1>
                                <a href="/log?username=${username}">
                                    <button>/log</button>
                                </a>

                                <a href="/sayHello?username=${username}">
                    ...
                    else
                    {
                        ...
                        response.end(`<h1>Usuario o contraseña incorrectos</h1>`);
                    }
            } 
            catch (err) 
            {
...
function log_handler(request, response)
{
    ...
    if (!session || session.status != 'enabled')
        {
            response.end("Sesión inválida");
            return;
        }
    if (authorize(username, '/log'))
    {
        response.end("Endpoint /log ejecutado correctamente");
    }
    else
    {
...
function say_hello_handler(request, response)
{
    ...
    if (authorize(username, '/sayHello'))
    {
        response.end("Endpoint /sayHello ejecutado correctamente");
    }
    else
    {
        response.end("Acceso denegado");
...
async function logout_handler(request, response)
{
    if (request.method == "POST")
    {
        ...
        request.on('end', () =>
        {
            ...
            catch (err)
            {
                ...
                response.end(`<h1>Error al cerrar sesión</h1>`);
            }
        });
    }
    else
    {
        ...
        response.end(`<h1>Método no permitido</h1>`);
    }
}
